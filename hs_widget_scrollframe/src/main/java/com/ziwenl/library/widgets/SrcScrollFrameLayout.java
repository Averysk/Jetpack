 ...
 * 3.可通过自定义属性 scrollSpreed 调整滚动速度，提供 slow、ordinary 和 fast 选项，也可自行填入 int 值，值越大滚动速度越快，建议 1 ≤ scrollSpreed ≤ 50
...
public class SrcScrollFrameLayout extends FrameLayout {
    ...
    public SrcScrollFrameLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        int scrollSpreed = array.getInteger(R.styleable.SrcScrollFrameLayout_scrollSpeed, 3);
        mScrollOrientation = array.getInteger(R.styleable.SrcScrollFrameLayout_scrollOrientation, 0);
        mIntervalIncreaseDistance = scrollSpreed * mIntervalIncreaseDistance;
